...
last_state = "CLEAR"
last_animal_time = 0
CLEAR_DELAY = 2  # seconds

def send_alert(state):
    global last_state, last_animal_time

    if state == "ALERT":
        mqtt_client.publish(ROAD_TOPIC, "ALERT", retain=True)
        last_state = "ALERT"
        last_animal_time = time.time()

    elif state == "CLEAR":
        if last_state == "ALERT" and (time.time() - last_animal_time) > CLEAR_DELAY:
            mqtt_client.publish(ROAD_TOPIC, "CLEAR", retain=True)
            last_state = "CLEAR"
...
@app.route("/dashboard")
def dashboard():
    # User exited detection → reset alert state
    mqtt_client.publish(ROAD_TOPIC, "CLEAR", retain=True)
    global last_state
    last_state = "CLEAR"
